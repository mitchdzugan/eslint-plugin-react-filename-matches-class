...
module.exports = {
  ...
  create: Components.detect(function(context, components) {
     ...
     */
    function reportNonMatchingComponentName(node, rawName, filename) {
      context.report(
        node,
        'Component name ' + rawName + ' does not match filename ' + filename
      );
    }
      ...
      'Program:exit': function() {
        ...
        for (var component in list) {
          if (list.hasOwnProperty(component)) {
            ...
            if (name !== null && name === filenameWithoutExtension) {
							foundMatch = true;
            }
          }
        }
						...
						if (list.hasOwnProperty(component)) {
							...
							if (name !== null && name !== filenameWithoutExtension) {
								reportNonMatchingComponentName(
									node, name, filenameWithoutExtension);
							}
						}
					}
				}
